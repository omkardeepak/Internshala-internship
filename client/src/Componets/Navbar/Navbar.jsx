...
const [value, setValue] = useState(null);
  ...
  const loginFunction = async () => {
    try {
      ...
      setValue(data.user.email);
      localStorage.setItem("email", data.user.email);
      setDivVisibleFrologin(false); // This should be inside the try block to ensure it's set only when login is successful
    } catch (error) {
      console.error(error);
    }
  };
    ...
    </div>
    {
        value?(
            ...
            <div className='Profile'>
                <Link to={"/profile"}>
                    <img src={value?.photo}  alt="" onMouseEnter={showtheProfile} className='rounded-full w-12' id='picpro' />
...
<p onClick={loginFunction} className='flex
 items-center h-9 justify-center mt-4 text-white bg-slate-100 rounded-lg hover:bg-gray-100' >
    ...
    <button className='text-gray-500' onClick={loginFunction}>Login With Google 
    </button>
 </p>
...
<div className="w-full p-8 lg:w-1/2">
             ...
             <div className="mt-8">
             <button className='btn3  bg-blue-500 h-9 text-white font-bold py-2 px-4 w-full rounded hover:bg-blue-600 '>Logi</button>
             </div>
...
<p className='text-sm'>new to internarea? Register(<span className='text-blue-500 cursor-pointer' onClick={closeLogin}>Student</span>/<span className='text-blue-500 cursor-pointer' onClick={closeLogin}>company</span>) </p>
    ...
    {isDivVisibleForProfile&&(
        <div className="profile-dropdown h-16 rounded-sm shadow-sm">
            ...
            <p className='font-medium'>{value?.email}</p>
